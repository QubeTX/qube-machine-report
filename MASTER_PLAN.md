...
**Last updated:** 2026-05-14 (Four-installer Windows distribution model, v3.15.0)
...
| v3.15.0 | (pending) | 2026-05-14 | **Four-installer Windows distribution model + MSI/EXE-aware self-update.** Adds three new Windows installer artifacts to every release: Corporate MSI (`wix/corporate.wxs`, perUser scope, no UAC, installs to `%LocalAppData%\Programs\tr300\bin\`), Global EXE installer (`inno/global.iss`, Inno Setup, perMachine, same path as Global MSI), and Corporate EXE installer (`inno/corporate.iss`, Inno Setup, perUser, same path as Corporate MSI). Six new permanent GUIDs total (two MSI UpgradeCodes, two Inno AppIds, two MSI registry-marker Component GUIDs). All four installers write a `HKCU\Software\TR300\InstallSource` registry value (`msi-global` / `msi-corporate` / `exe-global` / `exe-corporate`) which `src/update.rs::detect_install_origin()` reads to dispatch `tr300 update` to the matching installer for in-place upgrades — MSI strategies use `msiexec /i /passive /norestart`, EXE strategies use `setup.exe /SILENT /SUPPRESSMSGBOXES /NORESTART`. Path-based fallback (Program Files → MsiGlobal, LocalAppData\Programs → MsiCorporate, .cargo\bin → CargoOrInstaller) handles legacy installs without the marker. New `.github/workflows/windows-installers.yml` (triggered on `release: types: [published]`) builds the three new artifacts after cargo-dist's `release.yml` finishes; total release asset count is now 28. New `winreg = "0.52"` dependency for the registry read. README leads with the four direct-download installer links by edition, demotes `cargo install` to a "Build from source" subsection. Long-form rationale in `docs/architecture-decisions.md` § "Windows distribution model (v3.15.0+)". |
