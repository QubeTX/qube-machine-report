...
- **`.github/workflows/windows-installers.yml` is hand-authored.** It follows the exact successful `Release` workflow through `workflow_run`, because a public-release event is both too late for draft-first assembly and suppressed when the upstream workflow uses `GITHUB_TOKEN`. Its resolver binds the exact workflow ID/path/event/repository/SHA/attempt, immutable stable tag, Release target, current `main`, and CI before building. The read-only builder invokes `scripts/install-pinned-inno-setup.ps1`, which downloads official JRSoftware Inno Setup 6.7.3 and verifies fixed size/SHA-256, GitHub attestation, Pyrsys B.V. Authenticode, ProductVersion, and ISCC version/path; Chocolatey is not a trusted installer source. A fresh checkout-free `release-publishing` job adds exactly six fixed files to the private 24-asset draft, producing 30. `windows-installer-validation.yml` freezes and tests those exact bytes before publication. Only the fresh macOS finalizer may consume that proof, add four PKG/DMG files, and publish the exact 34-asset release. Crates publication is unrelated to the tag chain and automatically follows successful exact-main CI through protected OIDC.
