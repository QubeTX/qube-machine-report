...
- **Preserve `.github/workflows/release.yml` customizations.** cargo-dist v0.31.0 generates the base file, but the checked-in workflow contains two protected zones: legacy installer-alias copies and fail-closed Developer ID signing/Apple notarization before Mac artifact upload. New supplemental Windows installers belong in `windows-installers.yml`; any regeneration must restore both protected zones and repeat the Mac gate.
