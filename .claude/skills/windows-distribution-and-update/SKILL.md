...
**Windows (v3.15.0+).** `detect_install_origin()` reads `HKCU\Software\TR300\InstallSource` (written by all four first-class installers — MSI Global, MSI Corporate, EXE Global, EXE Corporate) and accepts it only when its Global/Corporate scope matches the running executable path. A valid marker returns one matching strategy. No cross-fallback: re-running a different product would create coexistence problems (two ARP entries, PATH ordering wins). The MSI strategies download the matching `.msi` to a private randomized `%TEMP%` directory and run `msiexec /i /passive /norestart`; the EXE strategies download the matching Inno Setup `.exe` and run it with `/SILENT /SUPPRESSMSGBOXES /NORESTART`. If the marker is missing or stale, `classify_install_path()` maps only `\.cargo\bin\` to `CargoOrInstaller`; marker-free Program Files, LocalAppData, and other locations are `Unknown`. A path identifies edition scope but cannot distinguish MSI from Inno EXE, so guessing would create a cross-format registration. `CargoOrInstaller` and `Unknown` use the conservative legacy chain.
...
- `verify_checksum(installer_path, installer_url)` runs after `download_to_file` and before invoking msiexec / the Inno EXE. It fetches `{installer_url}.sha256` in a separate request (cargo-dist and `windows-installers.yml` publish the same standard format), parses it, computes SHA-256 of the downloaded file, and refuses to launch on mismatch. This detects corruption or an installer/sidecar mismatch. Because both arrive through the same HTTPS release origin, the sidecar is not an independent signature and does not by itself defeat a proxy or origin compromise. Don't skip the integrity check or overstate it as authenticity.
...
**Post-install verification (v3.16.0+).** The **cargo** strategy (`cargo install tr300 --force`, used on macOS/Linux and Windows cargo installs) uses shared `reexec_installed_version()` + `post_install_version_ok()` through `verify_cargo_post_install(latest)`. `cargo install` can report success while crates.io still serves the old version, so mismatch falls through to the prebuilt release installer. Curl/wget/PowerShell terminal strategies also verify the running executable and report an honest failure if their default bindir did not replace it; never claim success from an installer exit code alone.

**Private staging and policy blocks (v4.0.0+).** MSI/EXE payloads live under a
randomized `tempfile::TempDir`; `finish_staged_attempt` explicitly closes it and
adds a diagnostic when endpoint scanning still holds the file. A verified
successful update remains success with a cleanup warning. Errors that strongly
indicate antivirus, Group Policy, execution policy, permissions, sharing locks,
virus removal, or MSI policy code 1625 become `PolicyBlocked`: record `blocked`,
stop the fallback chain, retain the current install, return exit 2, and show
official/manual URLs. Do not add a force prompt, direct running-binary
replacement, backup bypass, or another write-heavy attempt after a block.
...
- **`src/update.rs::detect_install_origin()` is the only place that decides which installer to fetch.** Install paths participate in scope validation through `is_global_install_path()` / `is_corporate_install_path()`; `classify_install_path()` recognizes `.cargo\bin` and otherwise returns `Unknown`. If paths change in `wix/main.wxs`, `wix-corporate/corporate.wxs`, or `inno/*.iss`, update those scope checks in lockstep. Never infer MSI versus EXE from a shared edition path.
...
- **`.github/workflows/windows-installers.yml` is hand-authored.** It follows the exact successful `Release` workflow through `workflow_run`, because a public-release event is both too late for draft-first assembly and suppressed when the upstream workflow uses `GITHUB_TOKEN`. Its resolver binds the exact workflow ID/path/event/repository/SHA/attempt, immutable stable tag, Release target, current protected `main`, and CI before building. The read-only builder invokes `scripts/install-pinned-inno-setup.ps1`, which downloads official JRSoftware Inno Setup 6.7.3 and verifies fixed size/SHA-256, GitHub attestation, Pyrsys B.V. Authenticode, ProductVersion, and ISCC version/path; Chocolatey is not a trusted installer source. A fresh checkout-free `release-publishing` job adds exactly six fixed files to the private 24-asset draft, producing 30. `windows-installer-validation.yml` freezes and tests those exact bytes before publication. Only the fresh macOS finalizer may consume that proof, add four PKG/DMG files, and publish the exact 34-asset release. Crates publication is unrelated and manual through the protected OIDC workflow; never reintroduce a CI `workflow_run` crate chain.
...
- **Preserve `.github/workflows/release.yml` customizations.** cargo-dist v0.31.0 generates the base file, but the checked-in workflow adds stable-tag/main/CI provenance, pinned installers/actions, managed and legacy aliases, fresh Apple signing, fixed 24-asset preparation, and a checkout-free private-draft publisher. New supplemental Windows installers belong in `windows-installers.yml`; regeneration must restore every boundary and rerun the workflow-provenance plus native Mac gates.
- **HKCU is intentional for the registry marker, even for perMachine installs.** `tr300 update` runs as the user and reads HKCU naturally; HKLM would complicate per-user/per-machine authoring. If an admin installs for a different user and no valid marker exists, the origin is deliberately `Unknown` because Program Files identifies Global scope but not MSI versus EXE. The conservative legacy/manual path is safer than cross-format guessing.
...
- **`Cargo.toml` `allow-dirty = ["ci", "msi"]`** is required: `"ci"` covers the legacy aliases and Apple trust step in the generated/check-in release workflow; `"msi"` covers customized `wix/main.wxs` Component additions. Both flags must stay.
