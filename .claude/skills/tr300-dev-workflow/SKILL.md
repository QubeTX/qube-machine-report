...
- **`.github/workflows/crates-publish.yml`** — manual owner-only publication from exact protected `main` after CI. A tokenless Cargo 1.95 validator proves the package; a fresh `crates-io` runner executes no package code while a short-lived OIDC token exists and verifies public bytes/provenance. Automatic push/`workflow_run` publication is forbidden.
